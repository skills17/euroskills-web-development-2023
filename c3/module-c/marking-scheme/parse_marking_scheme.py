...
target_wsss = [0]*10
target_wsss_total = 0
target_wsss_descriptions = [""]*10
import csv
with open('MARKING_DISTRIBUTION.csv') as csvfile:
    csv_reader = csv.reader(csvfile)
    for row in csv_reader:
        mark = float(row[-1])
        target_wsss_descriptions[int(row[0])] = row[1]
...
for index, wsss_mark in enumerate(wsss_marks):        
    ...
    wsss_description = target_wsss_descriptions[index]
    print(f"{index}\t{wsss_mark}\t\t{target_mark}\t\t{wsss_mark-target_mark}\t{wsss_description}")
